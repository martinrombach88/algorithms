...
public class Main {
        //1. Make a type (class, record etc.) that can be used in an itinerary
...
//    }
    ...
    public static LinkedList convertToLinkedList(ArrayList<Town> arrayList) {
        ...
        //Rule: You don't want to allow duplicate places to be in your list, for this data set.
    ...
    public static void moveLocation(LinkedList placesToVisit, ListIterator iterator, boolean forward) {
        ...
        if (!iterator.hasPrevious()) {
            ...
            prevTown = (Town) placesToVisit.get(placesToVisit.size() -2);
            town = (Town) placesToVisit.getLast();
        } else {
            prevTown = (Town) placesToVisit.get(iterator.previousIndex());
            town = (Town) placesToVisit.get(iterator.previousIndex() + 1);
        }
    ...
    public static void printFullItinerary(LinkedList<Town> placesToVisit) {
        ...
        for (int i = 1; i < placesToVisit.size(); i++){
            System.out.printf("->  From %s to %s, distance: %dkm. %n",placesToVisit.get(i-1).name,placesToVisit.get(i).name, placesToVisit.get(i-1).distance);
        }
    }
}
